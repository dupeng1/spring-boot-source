...
public abstract class AutoConfigurationPackages {
	...
	public static void register(BeanDefinitionRegistry registry, String... packageNames) {
		//这里参数packageNames缺省情况下就是一个字符串，是使用了注解@ApringBootApplication的SpringBoot应用程序入口类所在的包
		if (registry.containsBeanDefinition(BEAN)) {
			...
			//如果该bean尚未注册，则注册该bean，参数中提供的包名会被设置到bean定义中去
	 ...
	 * configuration.
	 */
		//该类就干一个事，注册一个Bean，这个Bean就是BasePackages，它有一个参数，这个参数是使用了@AutoConfigurationPackage这个注解的类（核心启动类）
		//所在的包路径，保存自动配置类以供之后的使用，比如给JPA entity扫描器用来扫描开发人员通过注解@Entity定义的entity类
